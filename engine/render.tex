...
%% Sideways bleed.  Normally one overlap; zeroed for the rightmost column,
%% whose overflow would have nothing to paint over and would instead show as
%% a fringe on the page behind.
\dim_new:N \l_dtex_wover_dim
\dim_set:Nn \l_dtex_wover_dim { \c_dtex_overlap_dim }
    ...
    \int_compare:nNnT {#3} > {0}
      {
        \hbox { \textcolor { \dtexPC {#1} {#2} }
          { \vrule width \dim_eval:n { \c_dtex_px_dim + \l_dtex_wover_dim }
...
%% Downward bleed for a band whose lower edge is at screen row #1: none at
%% the very bottom of the viewport, for the same reason.
\cs_new:Npn \dtexBleed #1
  {
    \int_compare:nNnTF {#1} < { \c_dtex_screen_h_int }
      { \c_dtex_overlap_dim } { 0pt }
  }
    ...
    \int_step_inline:nnn {0} {7}
      {
        ...
        \dtexBandOv { \c_dtex_ceil_pal_int } {##1}
          { \l_dtex_b_int - \l_dtex_a_int } { \dtexBleed { \l_dtex_b_int } }
      }
  }
    ...
    \int_step_inline:nnnn {7} {-1} {0}
      {
        ...
        \dtexBandOv { \c_dtex_floor_pal_int } {##1}
          { \l_dtex_b_int - \l_dtex_a_int } { \dtexBleed { \l_dtex_b_int } }
      }
  }
    ...
    \hbox
      {
        \int_step_inline:nnn {0} { \c_dtex_screen_w_int - 1 }
          {
            \int_compare:nNnT {##1} = { \c_dtex_screen_w_int - 1 }
              { \dim_set:Nn \l_dtex_wover_dim {0pt} }
            \dtexRenderColumn {##1}
            \kern -\l_dtex_wover_dim
          }
        \dim_set:Nn \l_dtex_wover_dim { \c_dtex_overlap_dim }
      }
  }
